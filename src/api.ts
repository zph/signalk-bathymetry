...
export function registerRoutes(router: PluginRouter, getRuntime: () => Runtime | undefined): void {
  ...
  read.get('/tiles/:z/:x/:y.png', (request, response) => {
    ...
    try {
      ...
      const etag = `W/\"hex4-${runtime.store.revision()}-${z}-${x}-${y}-${layer}-${mode}-${tideBucket}-${Number(runtime.config.showDepthLabels)}-${runtime.config.depthLabelMinZoom}-${unitStatus.revision}\"`
...
export function openApi(): object {
  return {
    openapi: '3.0.3',
    info: { title: 'Signal K Local Bathymetry API', version: '0.2.2' },
    paths: {
